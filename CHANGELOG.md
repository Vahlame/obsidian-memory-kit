...
- **Simpler initializer install (`@vkmikc/create-obsidian-memory` → 3.1.0).** The vault path is now optional: pass it as a **positional argument** (`npx @vkmikc/create-obsidian-memory ./vault -y`) or omit it to default to `~/Documents/cursor-memory-vault`. `-y` is a new alias for `--non-interactive`/`--yes`, and a missing vault is now **created** (starter notes) instead of erroring out. The old `-- --non-interactive --vault "<path>"` form still works; docs now show the short form.
