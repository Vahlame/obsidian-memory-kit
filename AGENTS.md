...
**Evolve and self-check.** Form hypotheses about recurring patterns — track tech in `STACKS/` (one note each), log likely high-impact anti-patterns in the user's code to `PRACTICES/observations.md`, and promote to `PRACTICES/confirmed-{good,bad}.md` only when the user confirms (coach by asking, never impose). Before a non-trivial answer, do a brief internal self-check (assumptions, edge cases, what would make this wrong) scaled to the task. Keep it cheap — passage-first, terse bullets, no long monologues.
